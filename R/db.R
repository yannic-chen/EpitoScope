...
.db_path <- function() getOption("epitoscope.db", "epitoscope_results.sqlite")
.db_con <- function(path = getOption("epitoscope.db", "epitoscope_results.sqlite"))
...
save_analysis_to_db <- function(lst, meta_table, quantity_cols, col_map = NULL,
                                ...
                                allow_duplicate = FALSE, path = .db_path()) {
...
seed_condition_terms <- function(con, seed_path = system.file("extdata", "condition_seed.csv", package = "EpitoScope")) {
  ensure_condition_terms(con)
...
# ------- check data ----
list_analyses <- function(path = .db_path()) {
...
load_run_data <- function(run_id, path = .db_path()) {
  con <- .db_con(path); on.exit(DBI::dbDisconnect(con))
...
# ---------browse database--------------
db_tables <- function(path = .db_path()) {
  if (!file.exists(path)) return(character(0))
  con <- .db_con(path); on.exit(DBI::dbDisconnect(con))
  DBI::dbListTables(con)
}

db_schema <- function(path = .db_path()) {
  con <- .db_con(path); on.exit(DBI::dbDisconnect(con))
...
# ---------load from database--------------
load_analysis_from_db <- function(aid, path = .db_path()) {
  con <- .db_con(path); on.exit(DBI::dbDisconnect(con))
...
#' Ensure the condition-terms table exists and is seeded (idempotent).
#' @keywords internal
seed_db_once <- function() {
  con <- .db_con(); on.exit(DBI::dbDisconnect(con))
  ensure_condition_terms(con)
  seed_condition_terms(con)
}
