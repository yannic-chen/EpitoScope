...
app_server <- function(input, output, session, input_variable, generate_pseudo_sequence = FALSE, custom_schema = NULL, custom_signature = NULL, replace_schema = FALSE) {
  # plotly::ggplotly() calls grDevices::dev.size(); with no device open,
  # R would spawn a file device (Rplot001.jpg). Open a null device for the
  # app's lifetime so dev.size() always has one to query, and close it on exit.
  if (is.null(grDevices::dev.list())) {
    grDevices::pdf(NULL)
    session$onSessionEnded(function() try(grDevices::dev.off(), silent = TRUE))
  }
#------------------State Check---------------------
  ## State container
  ...
  observeEvent(input$generate_report, {
    ...
    tryCatch({
      ...
      out_path <- file.path(tempdir(), out_file)
          ...
          saveRDS(params, "dev/report_params.rds")
      # Show progress
      withProgress(message = "Generating report...", value = 0, {
        rmarkdown::render(
          input = system.file("report", "report.Rmd", package = "EpitoScope"),
          output_file = out_path,
          params = params,
          envir       = new.env(parent = asNamespace("EpitoScope")),
        )
